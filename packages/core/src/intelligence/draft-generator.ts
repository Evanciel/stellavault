...
export interface BlueprintSection {
  title: string;
  tags?: string[];      // 이 챕터에 포함할 태그 필터
  description?: string; // 챕터 목적
}
...
export interface DraftOptions {
  ...
  maxSections?: number;
  blueprint?: BlueprintSection[];  // 챕터 구성 입력
}
  ...
  if (blueprint && blueprint.length > 0) {
    ...
    for (const section of blueprint) {
      ...
      for (const doc of usedDocs) {
        ...
        if (excerpt) bpLines.push(`> ${excerpt}\n> — *${doc.title}*\n`);
      }
