...
// bounded (limit is FETCH_LIMIT=30 at the call site → 300 rows).
  ...
  return {
    ...
    async searchSemantic(embedding: number[], limit: number): Promise<ScoredChunk[]> {
      ...
      const rows = db.prepare(`
        ...
        LIMIT ?
      `).all(float32Buffer(embedding), limit * DIVERSITY_OVERFETCH, limit) as Array<{ chunk_id: string; distance: number }>;
    ...
    async searchKeyword(query: string, limit: number): Promise<ScoredChunk[]> {
      ...
      // 안쪽 LIMIT 로 창을 묶어 두어 윈도우 함수가 전체 매치를 훑지 않게 한다.
      const rows = db.prepare(`
        ...
        LIMIT ?
      `).all(query, limit * DIVERSITY_OVERFETCH, limit) as Array<{ chunk_id: string; rank: number }>;
