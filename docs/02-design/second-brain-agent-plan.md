...
네이티브 `/api/chat` args는 **객체**라 `JSON.parse` 경로 없음(인젝션·double-parse throw 없음). 단 향후 `/v1` 또는 클라우드 폴백이 들어오면 args는 **문자열**이고 라우터가 `finish_reason=length`를 `tool_calls`로 가장할 수 있다 → **args 문자열이 `}`/`]`로 안 끝나면 거부+partial 반환**(실행 금지). 이 게이트를 디스패처에 **지금 심어**(no-op for native) 폴백 경로가 회귀 못 하게 한다.
