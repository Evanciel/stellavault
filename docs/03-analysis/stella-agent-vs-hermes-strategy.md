...
**BUILD (이걸로 조합을 이긴다)** — 진행현황 2026-06-29 (울트라코드 wf_b42edb00 설계+적대리뷰)
1. **frontier 기본 경로화 + 라이브 E2E** (PR#14 마감): 로컬=프라이버시 기본, frontier=품질 부스트. ⛔ **차단=키·유료 라이브 E2E(사용자 단계) + PR#14 미머지**. (리뷰: 모든 ChatView/온보딩 작업은 PR#14 머지 후가 이상적)
2. **plan-act-reflect 깊이**: ⚠️ **REVISE — 보류(결정 필요).** 적대리뷰가 치명 발견: 미완성 답변이 화면에 스트리밍된 뒤 reflect 재답변이 **같은 버블에 이중표시**(single-settle 핫스팟 L1261). 깔끔히 하려면 (a)이중표시 수용(추함) 또는 (b)렌더러에서 partial 억제(스코프↑). + 이름충돌(기존 `reflect` 옵션). **결정 전 구현 안 함.**
3. **프로액티브 지식**: ✅ **DONE.** v1 프롬프트 넛지(`8b31a39`) + v2 데이터칩(`8b30f51`, chat:proactive-brief 읽기전용·title만·캐시·en/ko·467테스트). **핵심 차별점 출하.**
4. ~~write-approval 정책 노브~~ ✅ **이미 구현됨**(`confirmWrites`, audit P0-1 닫힘). 잔여=🤖 정직화(미머지 PR#14).
5. **제로셋업 온보딩**: 🟡 **부분.** 자동인덱싱·피커·챗진입은 이미 존재. 진짜 갭 2개: (A) **데모볼트 온램프** = 최고가치지만 **forge extraResource 패키징(asar 이력)·릴리즈 작업 → 데스크탑 릴리즈 정지와 충돌, 보류.** (B) 첫실행 챗 자동오픈 = 데모볼트 없으면 빈 챗이라 한계가치. → **데모볼트(A)는 릴리즈 재개 시.**
