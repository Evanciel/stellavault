...
本格的なマークダウンエディタ、3D ニューラルグラフ、AI 検索、間隔反復(spaced repetition)を備えた **自己コンパイル(self-compiling)型ナレッジベース**です。**デスクトップアプリ**、**CLI**、**Obsidian プラグイン**、**MCP サーバー**として利用でき、ボールト(vault)のファイルは決して変更されません。
